...
const bodyParser = require('body-parser'); //usado para o postman
const urlencodedParser = bodyParser.urlencoded({ extended: false }) //para pegar os elementos que vem do formulario via postman
...
app.post('/insereUsuario', urlencodedParser, (req, res) => {
    ...
    res.setHeader('Access-Control-Allow-Origin', '*');
    var db = new sqlite3.Database(DBPATH); // Abre o banco
    sql = "INSERT INTO usuarios (nome, cargo, sobre_mim, endereco, telefone, email) VALUES ('" + req.body.nome + "', '" + req.body.cargo + "', '" + req.body.sobre_mim + "', '" + req.body.endereco + "', '" + req.body.telefone + "', '" + req.body.email + "')";
    console.log(sql);
    db.run(sql, [],  err => {
        if (err) {
            throw err;
...
app.get('/atualizaUsuario', (req, res) => {
	res.statusCode = 200;
	res.setHeader('Access-Control-Allow-Origin', '*'); 
	sql = "SELECT * FROM usuarios WHERE cod_usuario="+ req.query.cod_usuario;
	console.log(sql);
	var db = new sqlite3.Database(DBPATH); // Abre o banco
	db.all(sql, [],  (err, rows ) => {
		if (err) {
...
app.post('/atualizaUsuario', urlencodedParser, (req, res) => {
	res.statusCode = 200;
	res.setHeader('Access-Control-Allow-Origin', '*'); 
	sql = "UPDATE usuarios SET nome='" + req.body.nome + "', email = '" + req.body.email + "' , telefone='" + req.body.telefone + "' WHERE cod_usuario='" + req.body.cod_usuario + "'";
	console.log(sql);
	var db = new sqlite3.Database(DBPATH); // Abre o banco
	db.run(sql, [],  err => {
		if (err) {
